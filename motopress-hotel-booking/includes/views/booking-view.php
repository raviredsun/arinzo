...
class BookingView {
	...
	public static function generatePriceBreakdown( Entities\Booking $booking ){
		...
		return self::generatePriceBreakdownArray( $priceBreakdown,array("booking_id" => $booking->getId()) );
	}
	/**
	...
	public static function generatePriceBreakdownArray( $priceBreakdown, $atts = array() ){
		...
		if ( !empty( $priceBreakdown ) ) :
            ...
            $products_title = '';
            $products_price_total = '';
            $paytype = '';

            if(!empty($atts['booking_id'])){
            	$products_title = get_post_meta($atts['booking_id'],"products_title2",1);
            	$products_price_total = get_post_meta($atts['booking_id'],"products_price_total",1);
            	$paytype = get_post_meta($atts['booking_id'],"paytype",1);
            }
            if(!$products_title && !empty($_GET['post'])){
            	$products_title = get_post_meta($_GET['post'],"products_title2",1);
            	$products_price_total = get_post_meta($_GET['post'],"products_price_total",1);
            	$paytype = get_post_meta($_GET['post'],"paytype",1);
            }
			?>
				...
				<tbody>
					<?php foreach ( $priceBreakdown['rooms'] as $key => $roomBreakdown ) : ?>
						...
						<?php if($products_title) { ?>
							<tr class="<?php echo esc_attr("{$foldedClass} mphb-price-breakdown-product"); ?>">
								<th colspan="<?php echo ( $useThreeColumns ? 3 : 2 ); ?>">
									<?php _e( 'Products', 'motopress-hotel-booking' ); ?>
								</th>
							</tr>
							<tr class="<?php echo esc_attr("{$foldedClass} mphb-price-breakdown-product-price"); ?>">
								<th colspan="<?php echo ( $useThreeColumns ? 2 : 1 ); ?>">
									<?php echo $products_title; ?>
								</th>
								<th class="mphb-table-price-column">
									<?php echo mphb_format_price($products_price_total); ?>
								</th>
							</tr>
						<?php  $roomBreakdown['discount_total'] = $roomBreakdown['discount_total'] + $products_price_total; } ?>
						...
						<?php if($paytype == "not_refundable"){ ?>
							<tr class="<?php echo esc_attr("{$foldedClass} mphb-price-breakdown-subtotal"); ?>">
								<th colspan="<?php echo ( $useThreeColumns ? 2 : 1 ); ?>"><?php _e( 'Non-Refundable Discount 5%', 'motopress-hotel-booking' ); ?></th>
								<th class="mphb-table-price-column">-<?php echo mphb_format_price( ($roomBreakdown['discount_total']*5)/100 ); ?></th>
							</tr>
						<?php } ?>
					<?php endforeach; ?>

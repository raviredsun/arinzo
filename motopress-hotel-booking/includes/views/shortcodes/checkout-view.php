...
class CheckoutView {
    ...
    public static function renderGuestsChooser($reservedRoom, $roomIndex, $roomType, $booking)
    {
        ...
        <p>
            ...
            <select name="mphb_beach_arrival_time" id="mphb_beach_arrival_time">
            <?php
        ...
        <p>
            ...
            <select name="mphb_lunch_time" id="mphb_lunch_time">
                <?php foreach ($lunch_time_list as $key => $value) { ?>
                    <?php 
                        if ($lunchtime == $value['id'] || $lunchtime == $value['title']){
        ...
        /*if(isset($_GET['checkk'])){
            ...
            if($booking_ids){
                ...
                foreach ($booking_ids as $key => $value) {
                    ...
                    $lunch_time = get_post_meta($value->post_id, 'lunch_time', true);
                    if($lunch_time == '13:00'){
                        $lunch_time = '13:15';
                    }
                    $ids = get_post_meta($value->post_id, 'arf_cp_table_id', true);
                    if($ids){
                        if(is_array($ids)){
                            foreach ($ids as $kk => $vv) {
                                if($vv)
                                    $array[$value->meta_value][$lunch_time][get_the_title($vv)][] = $value->post_id;
                            }
                        }else{
                            $array[$value->meta_value][$lunch_time][get_the_title($ids)][] = $value->post_id;
                        }
                    }
                }
            }
            ...
            foreach ($array as $date => $value) {
                foreach ($value as $time => $vv) {
                foreach ($vv as $table => $v) {
                    if(count($v) <= 1) continue;
                    foreach ($v as $kkk => $vvv) {
                        # code...
                        echo $date."#".$table."#".$time."#".$vvv."<br/>";
                    }
                }
                }
            }die;
        ...
        if(isset($_GET['checkk'])){
            ...
            if($booking_ids){
                ...
                foreach ($booking_ids as $key => $value) {
                    $booking_slot = array();
                    $mmm = get_post_meta($value->post_id, 'mphb_place', true);
                    if($mmm){
                        foreach($mmm AS $kk => $vv){
                            if(isset($booking_slot)){
                                $booking_slot = array_merge($booking_slot,$vv);
                            }else{
                                $booking_slot = $vv;
                            }
                        }
                    ...
                    foreach ($booking_slot as $kk => $vv) {
                        if($vv)
                            $array[$value->meta_value][$vv][] = $value->post_id;
                    }
            ...
            //echo "<pre>"; print_r($array); echo "</pre>";die; 
            foreach ($array as $date => $value) {
                foreach ($value as $location => $vv) {
                
                    if(count($vv) <= 1) continue;
                    foreach ($vv as $kkk => $vvv) {
                        # code...
                        echo $date."#".$location."#".$vvv."<br/>";
                    }
                }
            }die;
        }
        $lunch_time = get_post_meta($_GET["booking_id"], 'lunch_time', true);
        ...
        <p>
            ...
            <br>
            <select name="mphb_table_id[]" multiple="multiple" id="mphb_table_id">
		...
		<?php foreach($locations AS $key => $value){ $ddd = get_field("location_names",$value->ID); $arr = array(); $ddd = explode("|",$ddd); if($ddd){ foreach($ddd AS $kk => $vv){ $arr = array_merge($arr,explode(",",$vv)); } } ?>
		    <p>
                ...
                <br>
                <select name="mphb_place[<?php echo $value->ID; ?>][]" multiple="multiple" id="mphb_place_<?php echo $value->ID; ?>">
            ...
            <ul class="mphb_sc_checkout-services-list mphb_checkout-services-list">
                <?php foreach ($services as $index => $service) {
                    ...
                    if ($service->isPayPerAdult() && $roomType->getAdultsCapacity() > 1) {
                        ...
                        $presetAdults = apply_filters('mphb_sc_checkout_preset_service_adults', $reservedRoom->getAdults(), $service, $reservedRoom, $roomType);
                    }
                    $presetChild = apply_filters('mphb_sc_checkout_preset_service_child', $roomType->getChildrenCapacity(), $service, $reservedRoom, $roomType);
                    ...
                    <li>
                        ...
                        <?php if ($service->isPayPerAdult() && $roomType->getAdultsCapacity() > 1) { ?>
                            <label for="<?php echo $idPrefix; ?>-adults">
                                ...
                                <select name="<?php echo $namePrefix; ?>[adults]" id="<?php echo $idPrefix; ?>-adults" class="mphb_sc_checkout-service-adults mphb_checkout-service-adults" onchange="if(jQuery(this).parents('li').find('.mphb_checkout-service').prop('checked')){ jQuery('.adult_field').val(jQuery(this).val()); }">
                            ...
                            <label for="<?php echo $idPrefix; ?>-child">
                                <?php _e('for ', 'motopress-hotel-booking'); ?>
                                <select name="<?php echo $namePrefix; ?>[child]" id="<?php echo $idPrefix; ?>-child" class="mphb_sc_checkout-service-child mphb_checkout-service-child"  onchange="if(jQuery(this).parents('li').find('.mphb_checkout-service').prop('checked')){ jQuery('.child_field').val(jQuery(this).val()); }">
	...
	public static function renderCustomerDetails(){
		...
		<section id="mphb-customer-details" class="mphb-checkout-section mphb-customer-details">
			...
			<?php if ( MPHB()->settings()->main()->isRequireCountry() ) : ?>
				...
				<p class="mphb-customer-country" style="display: none">
			...
			<p class="mphb-customer-note" style="display: none">
    ...
    public static function renderProductChooser($reservedRoom, $roomIndex, $roomType, $booking)
    {
        ...
        $booking_id = isset($_GET["booking_id"]) ? $_GET["booking_id"] : 0;
        $products_qty = get_post_meta($booking_id,"products_qty",true);
        $products = get_post_meta($booking_id,"products",true);
        $products_title = get_post_meta($booking_id,"products_title",true);
        
        /*$booking = mphb_get_booking($booking_id, true);

        if(isset($_GET['a'])){
            echo "<pre>"; print_r($booking->getPriceBreakdown()); echo "</pre>";die; 
        }*/
        ...
        <section id="mphb-product-details-<?php echo esc_attr($roomIndex); ?>" class="mphb-product-details mphb-checkout-item-section">
            ...
            <div class="product_container">
                ...
                <?php  $checked = ""; if($features_image_type){ ?>

                    <?php foreach ($features_image_type as $key => $value) { if($value == "service") continue; 
                        $qty_stock = (int)get_post_meta($value,"stock",1);
                        $bottal_attribute = (int)get_post_meta($value,"bottal_attribute",1);
                        if(!$qty_stock) continue;
                        if($adultsTotal > 1 AND $bottal_attribute == "0.375") continue;
                        ?>
                                    ...
                                    <tbody>
                                        <tr>
                                            <td style="font-size: 12px;" class="product_title">
                                                <input onclick="jQuery('.products_disable').prop('checked',false)" type="radio" name="products[]" value="<?= $value ?>" <?php if($products && in_array($value, $products)) { echo "checked";$checked = 1;; } ?>>
                                            </td>
                ...
                <?php if(!$checked){ ?>
                    <table style="background: #ffffff;">
                        <tbody>
                            <tr>
                                <td style="font-size: 12px;" class="product_title">
                                    <input type="radio" class="products_disable" name="products_disable" value="" checked readonly="readonly" disabled="disabled">
                                </td>
                                <td style="font-size: 12px;color: #939393;" class="product_title"><?php echo $products_title ?></td>
                            </tr>
                        </tbody>
                    </table>
                <?php } ?>

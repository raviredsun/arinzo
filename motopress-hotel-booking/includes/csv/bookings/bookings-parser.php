...
class BookingsParser
{
    ...
    protected function parseProductName($booking, $room)
    {
        return strip_tags(get_post_meta($booking->getId(),"products_title2",1));
    }

    protected function parseLunch($booking, $room)
    {
        return get_lunch_text2(get_post_meta($booking->getId(),"lunch_time",1));
    }
    protected function parseLunch_time($booking, $room)
    {
        return "";
    }
    protected function parseLunchTime($booking, $room)
    {
        return "";
    }
    protected function parseProductNumber($booking, $room)
    {
        return get_post_meta($booking->getId(),"products_qty",1);
    }
    protected function parseProductPrice($booking, $room)
    {
        $qty = get_post_meta($booking->getId(),"products_qty",1);
        $price = get_post_meta($booking->getId(),"products_price_total",1);
        $price = ($qty && $price ? ($price/array_sum($qty)) : $price);
        return $price;
    }
    protected function parseProductQuantity($booking, $room)
    {
        $qty = get_post_meta($booking->getId(),"products_qty",1);
        return strip_tags($qty ? array_sum($qty) : "");
    }
    protected function parseProductTotal($booking, $room)
    {
        return strip_tags(get_post_meta($booking->getId(),"products_price_total",1));
    }
    protected function parseAccommodationPlace($booking, $room)
    {
    ...
    protected function parseTableSunbedAssigned($booking, $room)
    {
        ...
        $ids =  get_post_meta($booking->getId(),"arf_cp_table_id",true);;
        if(is_array($ids)){

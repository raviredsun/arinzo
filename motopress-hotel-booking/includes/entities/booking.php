...
class Booking {
	 ...
	 * @return array
	 */
	public function getPriceBreakdown($id = ""){
		if(!$this->id){
			$this->id = $id;
		}
		$coupon = null;
        ...
        if($this->id){
        	$products_price_total = get_post_meta( $this->id, 'products_price_total', true );
        	if($products_price_total){
        		$total += $products_price_total;
        	}
        	$paytype = get_post_meta( $this->id, 'paytype', true );

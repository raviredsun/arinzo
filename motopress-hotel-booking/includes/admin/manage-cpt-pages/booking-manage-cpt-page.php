...
class BookingManageCPTPage extends ManageCPTPage {
	...
	public function filterColumns( $columns ){
		...
		$customColumns = array(
			...
			'check_in_out_date'	 => __( 'Check-in', 'motopress-hotel-booking' ),
			...
			'beach_arrival_time'			 => __( 'Beach Arrival Time', 'motopress-hotel-booking' ),
			'lunch_time'			 => __( 'Lunch Time', 'motopress-hotel-booking' ),
			'notes'			 => __( 'Notes', 'motopress-hotel-booking' ),
		);
...
//				}
                        ...
                        <?php if ( $customer ) : ?>
                            <?php
                                ...
                                if ( !empty( $customer->getPhone() ) ) {
                                    ...
                                    $customerInfo.=' <a href="https://wa.me/' . esc_attr( $customer->getPhone() ) . '" target="_blank" style="border: none;"><img src="https://booking.arienzobeachclub.com/wp-content/uploads/2022/11/whatsapp-logo-light-green-png-0.png" width="30"  class="no-border"></a>';
                                }
			...
			case 'price':
				...
				$paytype = get_post_meta($booking->getId(),"paytype",true);

				if($paytype == "not_refundable"){
					echo '<br/>';
					echo "Pay Type : Not Refundable";
				}else if($paytype == "late_charge"){
					echo '<br/>';
					echo "Pay Type : Late Charge";
				}else if($paytype == "refundable"){
					echo '<br/>';
					echo "Pay Type : Refundable";
				}else if($paytype == "last_minute"){
					echo '<br/>';
					echo "Pay Type : Last Minute";
				}
					$paytype_text = "";
				$naxi_pay = get_post_meta($booking->getId(),"naxi_pay",true);
				if($naxi_pay){
					echo '<br/>';
					echo "Nexi Pay : €".($naxi_pay);
				}
				break;
            case 'room_type':
                ...
                $mphb_table_id = get_post_meta($booking->getId(), 'arf_cp_table_id', true);

                $mphb_place = get_post_meta($booking->getId(), 'mphb_place', true);
                 
                 	if($mphb_table_id){
                 		$table_name = array();
                 		foreach ($mphb_table_id as $key => $value) {
                 		...
                 		if($table_name){
                 			echo "<br/>Tables : ".implode(",", $table_name)."<br/>";
                 		}
                 	}
                 	if($mphb_place){
                 		echo "<br/>Location : ";
			...
			case 'mphb_date':
				...
				<?php
				break;
			case 'beach_arrival_time':
				echo get_post_meta($booking->getId(),"beach_arrival_time",true);
				break;
			case 'lunch_time':
				echo get_lunch_text(get_post_meta($booking->getId(),"lunch_time",true));
				break;
				
			case 'notes':
				echo get_post_meta($booking->getId(),"mphb_note",true);
				break;
            case 'services':
                $placeholder      = ' &#8212;';
                    ...
                    if (!empty($reservedServices)) {
                        ...
                        foreach ($reservedServices as $reservedService) {
                            ...
                            if ($reservedService->isPayPerAdult()) {
                                echo ' <em>' . sprintf(_n('x %d adult', 'x %d adults', $reservedService->getAdults(), 'motopress-hotel-booking'), $reservedService->getAdults()) . '</em>';

                                if($reservedService->getChild()){
                                	echo ' <em>' . sprintf(_n(', x %d children', ', %d childrens', $reservedService->getAdults(), 'motopress-hotel-booking'), $reservedService->getChild()) . '</em>';
                                }
                            }
				...
				$products = get_post_meta($booking->getId(),"products_title",true);
					
				if($products){
					echo "<div >Products : ".$products."</div>";
				}

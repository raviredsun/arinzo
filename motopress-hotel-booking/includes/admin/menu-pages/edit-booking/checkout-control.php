...
class CheckoutControl extends StepControl
{
    ...
    public function setup()
    {
        ...
        if(empty($_POST['check_in_date'])){
            $_POST['check_in_date'] = $this->editBooking->getCheckInDate()->format("d/m/Y");   
        }
        if(empty($_POST['check_out_date'])){
            $_POST['check_out_date'] =  $this->editBooking->getCheckOutDate()->format("d/m/Y");
        }
        // Parse dates
        ...
        if(!isset($_POST['map_rooms'])){
            $_POST['map_rooms'] = array();
        ...
        add_action('mphb_edit_booking_checkout_room_details', array('\MPHB\Views\Shortcodes\CheckoutView', 'renderProductChooser'), 50, 4);
        ...
        add_filter('mphb_sc_checkout_preset_service_child', array($this, 'presetServiceChild'), 10, 3);
    ...
    public function presetServiceAdults($adults, $service, $reservedRoom)
    {
        ...
        if (isset($this->roomDetails[$roomId]['presets']['services'][$serviceId])) {
            ...
            //echo "<pre>"; print_r($presetAdults); echo "</pre>";
            //echo "<pre>"; print_r($adultsCapacity); echo "</pre>";die; 
            if ($adultsCapacity != mphb_get_min_adults()) {
                return min((int)$presetAdults, (int)$adultsCapacity);
            }
        }
        return $adults;
    }
    
    public function presetServiceChild($child, $service, $reservedRoom)
    {
        $serviceId = $service->getOriginalId();
        $roomId = $reservedRoom->getRoomId();
        if (isset($this->roomDetails[$roomId]['presets']['children'])) {
            $child = $this->roomDetails[$roomId]['presets']['children'];
        }

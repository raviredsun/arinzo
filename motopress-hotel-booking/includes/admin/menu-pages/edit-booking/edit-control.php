...
class EditControl extends StepControl
{
    ...
    public function setup()
    {
        ...
        if(isset($_POST['check_out_date'])) {
            $date_custom = DateTime::createFromFormat("d/m/Y", $this->checkInDate->format("d/m/Y"));
            $date_custom->modify('+1 day');
            $checkOutDate_custom = $date_custom->format("d/m/Y");
            $this->checkOutDate = ParseUtils::parseCheckOutDate($checkOutDate_custom, array('check_booking_rules' => false, 'check_in_date' => $this->checkInDate));
        }
